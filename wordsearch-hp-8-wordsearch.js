...
;(function() {
  var LETTERS = 'abcdefghijklmnopqrstuvwxyz'; // letters used for filler - needs appending with accented chars depending on locality from config
  var WORD_RE = /^[a-z]+$/;                   // what a valid word looks like
  // var WORD_RE = /^[a-zÀ-ú]+$/;                   // what a valid word looks like - with accents
  // var WORD_RE = /^[a-zÀ-ÿ]+$/;                   // what a valid word looks like - with accents and umlauts
  ...
  function wordsearch(words, width, height, opts) {
    ...
    opts.diagonal = opts.hasOwnProperty('diagonal') ? opts.diagonal : true;
    ...
    for (var i = 0; i < words.length; i++) {
      ...
      while (attempts < MAXATTEMPTS) {
        ...
        var direction = opts.diagonal ? Math.floor(Math.random() * 4) : (Math.floor(Math.random() * 2) * 2) + 1;
